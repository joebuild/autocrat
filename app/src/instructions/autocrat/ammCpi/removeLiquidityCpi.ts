...
export const removeLiquidityCpiHandler = async (
  ...
  if (
    proposal.passMarketAmm.toBase58() !== ammAddr.toBase58() &&
    proposal.failMarketAmm.toBase58() !== ammAddr.toBase58()
  ) {
    throw new Error(
      "the amm address passed in removeLiquidityCpiHandler does not correspond to either the pass or fail market"
    );
  }
  ...
  let ix = await client.program.methods
    ...
    .accounts({
      ...
      conditionalMetaMint,
      conditionalUsdcMint,
      conditionalMetaUserAta: getATA(
        conditionalMetaMint,
        client.provider.publicKey
      )[0],
      conditionalUsdcUserAta: getATA(
        conditionalUsdcMint,
        client.provider.publicKey
      )[0],
